...
You can use a special notation to filter messages that you want to include in a run. The syntax mimics Discord's search query syntax, with additional capabilities.

Configure a filter in the GUI reaction dialog, or pass `--filter` in the CLI. See also [caveats](#cli-caveats).
...
```console
$ ./DiscordEmotify.Cli react [...] --filter "from:Tyrrrz has:image"
```

If you need to include quotes inside the filter itself as well, use single quotes (`'`) for those instead:

```console
$ ./DiscordEmotify.Cli react [...] --filter "from:Tyrrrz 'hello world'"
```
...
```console
$ ./DiscordEmotify.Cli react [...] --filter ~from:Tyrrrz
```
