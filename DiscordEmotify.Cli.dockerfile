...
COPY DiscordEmotify.Core DiscordEmotify.Core
COPY DiscordEmotify.Cli DiscordEmotify.Cli

# Publish a self-contained assembly so we can use a slimmer runtime image
RUN dotnet publish DiscordEmotify.Cli \
    -p:Version=$VERSION \
    ...
    --use-current-runtime \
    --arch $TARGETARCH \
    --output DiscordEmotify.Cli/bin/publish/
...
LABEL org.opencontainers.image.title="DiscordEmotify.Cli"
LABEL org.opencontainers.image.description="DiscordEmotify is a bulk reaction tool for Discord messages (CLI)."
LABEL org.opencontainers.image.authors="Otm02"
LABEL org.opencontainers.image.source="https://github.com/Otm02/DiscordEmotify"
...
COPY --from=build /tmp/app/DiscordEmotify.Cli/bin/publish /opt/app
