...
public class SettingsViewModel : DialogViewModelBase
{
    ...
    public int ReactionDelayMs
    {
        get => _settingsService.ReactionDelayMs;
        set => _settingsService.ReactionDelayMs = Math.Clamp(value, 0, 10000);
    }

    public IReadOnlyList<ReactionOrder> AvailableReactionOrders { get; } =
        Enum.GetValues<ReactionOrder>();

    public ReactionOrder ReactionOrder
    {
        get => _settingsService.ReactionOrder;
        set => _settingsService.ReactionOrder = value;
    }
